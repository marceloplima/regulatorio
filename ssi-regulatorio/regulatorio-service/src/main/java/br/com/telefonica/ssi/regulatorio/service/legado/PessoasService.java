...
@Stateless
public class PessoasService implements PessoasInt {
	...
	public Pessoas recuperarPorUsuario(Usuarios usuario) {
		...
		List<Pessoas> result = q.getResultList();
		if(result.size()<1){
			return null;
		}
	...
	@Override
	public List<Areas> retornarAreasPessoaAtividades(Pessoas pessoa){
		String jpaquery = "SELECT DISTINCT area FROM Areas area join fetch area.areaspessoas WHERE :pessoa in elements(area.areaspessoas) AND area.areasatividades IS NOT EMPTY order by area.cnmsiglaarea";
		TypedQuery<Areas> q = em.createQuery(jpaquery,Areas.class);
		q.setParameter("pessoa", pessoa);
		return q.getResultList();
	}
	...
	public String retornaTelefone(List<Telefones> telefones,Long idTipoTelefone) {
		for (Telefones telefone : telefones) {
			...
			if (telefone.getTipotelefone().getId()
